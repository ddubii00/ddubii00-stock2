...
const appBasePath = new URL(".", window.location.href).pathname.replace(/\/$/, "");

function apiUrl(endpoint) {
  return `${appBasePath}${endpoint}`;
}
...
function currentSortOptions() {
  const options = [
    ...
    { label: "PEG", value: "peg" },
    { label: "Forward PEG", value: "forwardPeg" },
    { label: "ROE", value: "roe" },
    { label: "PBR", value: "pbr" },
    ...
    .map((stock) => {
      ...
      return `
        <tr>
          ...
          <td class="numeric muted-value">${formatOptionalNumber(stock.forwardPeg, 2)}</td>
...
async function loadChartData() {
  ...
  const [response, ichiResponse] = await Promise.all([
    fetch(apiUrl(`/api/ohlcv?${params.toString()}`)),
    fetch(apiUrl(`/api/ohlcv?${ichiParams.toString()}`)),
  ]);
  const payload = await response.json();
...
async function loadStocks(forceRefresh = false) {
  ...
  try {
    ...
    const response = await fetch(
      apiUrl(`/${isScreener ? "api/screener" : "api/market"}?${params.toString()}`),
    );
    const payload = await response.json();
    if (!response.ok) {
